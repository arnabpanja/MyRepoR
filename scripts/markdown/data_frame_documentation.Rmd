...
Hello All !!! 
Another little **RMARKDOWN** document. Now, what has prompted me to take some time out of the busy schedule and create this document and share it with **R** Community? 

There are two drivers for it and they are 

#. The **comment** function in **R** that can help describe a column in a data frame
#. A question in the **R4DS Online Learning Community** on how to document a data frame. 

So we have a lot of methods of documenting our data model/data frame and we can use Excel to do it. There are few commercial tools out there in the market, which when given a set of database tables, can re-engineer an entity relationship diagram. 
But the question is, given that we do NOT want to use Excel and we also do NOT want to use any commercial tool but still if we want to have some basic level of documentation using our favorite language in **R** then what could be a plausible approach.

This document may shed some light on this. 

So let us watch how we can go about creating a basic documentation of our data frames. An analysis involves a number of data frames and a documentation, accompanied by a model diagram does go a long way in making things self-explanatory. 


#### **Palmer Penguins Data Set** 

So let us load the Palmer Penguins dataset.  
...
Not let us come to the first magic, the use of **comment** function. 
...
```{r docuemnt_df, include=TRUE, echo=TRUE}
...
And how does the data frame look like after that ? 
...
Our documentation is now taking shape !!!.
...
The **dtype** column now shows the data type of the columns. 
...
Now that we have shown how a documentation looks like, we would check out how a data model diagram looks like and how we can build that in R. In the above analysis, we have used a few data frame to document the Palmer Penguins data set.
...
So, coupled with the data frame documentation table, the above diagram does give us a picture of the data frames involved, with a reasonable description of it's important parts and a model diagram showing the relationships between them. 

So this is how, without using Excel, without using any commercial tools we can still generate a basic documentation of our data frames. 
